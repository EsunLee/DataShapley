...
- 分类器 = PLOVAD decoder（2 层 `k=1` Conv1d + GELU，65,793 参数）；**无动量 SGD**，学习率经校准后冻结（不用 plovad 的 Adam 1e-3）
...
- **4× RTX 3090（24GB）**，driver 495.29.05（CUDA 11.5，装不了新 torch cu121，勿尝试升级驱动）
